...
**`font-variant-numeric: tabular-nums`가 SVG `<text>`로 상속되면 한글이 깨진다.** 차트 수치 정렬을 위해 전역에 이 속성을 걸면, Chromium 일부 빌드에서 작은 크기의 SVG 텍스트에서 한글 자모 결합이 무너진다 — "말"이 "밀"·"믈"로 렌더된다. 두부(口)처럼 명백히 깨지지 않고 **다른 글자로 읽히기 때문에** 육안 검수에서 놓치기 쉽다.

```css
svg text { font-variant-numeric: normal; }   /* 전역 tabular-nums 차단 */
```

수치 정렬이 필요한 SVG 텍스트에는 개별 요소에만 `tabular-nums`를 다시 부여한다. 한글이 섞인 라벨에는 쓰지 않는다.

**차트 도형이 값을 인코딩하는지 실측하라.** 렌더가 그럴듯해 보여도 도형이 데이터와 무관하게 그려질 수 있다 — 실제로 도넛 조각이 41/41/18%인데 레이블은 55/31/14%, 값이 64/41/30/22/13인 가로막대 5개가 전부 411px 동일 길이로 렌더된 사례가 있었다. 렌더 후 **막대 길이·조각 각도를 실측해 값의 비율과 대조**하라. 순위 역전 0건, 비율 오차 ±2% 이내.
